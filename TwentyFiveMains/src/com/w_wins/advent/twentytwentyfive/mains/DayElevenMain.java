...
public class DayElevenMain {
    static void main() {
        ...
        while (!heap.isEmpty()) {
            ...
            current.outputs().stream().sorted(Comparator.comparing(order)).forEachOrdered(child -> graph.merge(child, Paths.howPathsAreAffectedAt(current.label()).apply(graph.get(current)), Paths::add));
        }
